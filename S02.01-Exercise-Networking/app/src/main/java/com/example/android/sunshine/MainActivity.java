...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadWeatherData(){
        ...
        URL preferredLocationWeatherUrl = NetworkUtils.buildUrl(location);
        new QueryAPIsAsyncTask().execute(preferredLocationWeatherUrl);
    }

    // COMPLETED (5) Create a class that extends AsyncTask to perform network requests
    class QueryAPIsAsyncTask extends AsyncTask<URL,Object,String>{
        @Override
        protected String doInBackground(URL... urls) {
            URL url = urls[0];
            String queryAPIResult = null;
            try {
                queryAPIResult = NetworkUtils.getResponseFromHttpUrl(url);
            } catch (IOException e) {
                e.printStackTrace();
            }
